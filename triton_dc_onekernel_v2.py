"""Single-kernel DC v2 — same as v1 but with num_warps=2 for large W.

For W <= 112 (KL=128): uses V1 logic with num_warps=4.
For W > 112 (KL=256):  uses V1 logic with num_warps=2 to fit s_acc/a_acc in registers.

No HBM intermediates. s_acc/a_acc in registers. Fused sweep 1+2.
"""
...
def _dc_v2_kernel(
    ...
    """Same logic as v1: fused sweep1+2, last pair QK cached, register s_acc/a_acc."""
    ...
    # ═══ Sweep 1: all heads QK → s_acc (register). Cache last pair QK. ═══
    ...
    # Last pair: keep QK in registers
    ...
    # Last pair first (QK in registers)
    for lh in range(2):
...
class TritonDCOneKernel:
    """V2: same as V1 but auto-selects num_warps based on KL."""
    ...
    def forward(
        ...
        # Auto-select num_warps: fewer warps = more regs/thread for large KL
        nw = 2 if KL > 128 else 4
