"""Single-kernel post-only DC attention v2.
...
- for D=128 and HPG=16, final AV can use a wide2 path that consumes one
  shared a_acc and produces two heads per dot
- the last head keeps direct PV in registers and stores final output once
...
def _post_only_onekernel(
    ...
    n_tail = (head_start + HPG - 1).to(tl.int64)
    q_ptrs = (Q + b * stride_qb + q_offs[:, None].to(tl.int64) * stride_qt
              + n_tail * stride_qn + d_offs[None, :].to(tl.int64) * stride_qd)
    q_blk = tl.load(q_ptrs, mask=q_mask[:, None], other=0.0)
    k_ptrs = (K + b * stride_kb + k_offs[:, None].to(tl.int64) * stride_kt
              + n_tail * stride_kn + d_offs[None, :].to(tl.int64) * stride_kd)
    k_blk = tl.load(k_ptrs, mask=k_mask[:, None], other=0.0)
    ...
    rs = tl.sum(p, axis=1)
    sl = tl.where(rs > 0.0, rs, 1.0)
    probs_tail = p / sl[:, None]

    v_tail_ptrs = (V + b * stride_vb + k_offs[:, None].to(tl.int64) * stride_vt
                   + n_tail * stride_vn + d_offs[None, :].to(tl.int64) * stride_vd)
    v_tail = tl.load(v_tail_ptrs, mask=k_mask[:, None], other=0.0)
    pv_tail = tl.dot(probs_tail.to(v_tail.dtype), v_tail)

    pdd_tail = tl.load(POST_DD + b * stride_wb + q_offs.to(tl.int64) * stride_wt + n_tail * stride_wn,
                       mask=q_mask, other=0.0).to(tl.float32)
    pw1_tail = tl.load(POST_W1 + b * stride_wb + q_offs.to(tl.int64) * stride_wt + n_tail * stride_wn,
                       mask=q_mask, other=0.0).to(tl.float32)
    a_acc += pw1_tail[:, None] * probs_tail

    a_acc = a_acc.to(tl.float16)

    av_tail = tl.dot(a_acc, v_tail)
    pw2_tail = tl.load(POST_W2 + b * stride_wb + q_offs.to(tl.int64) * stride_wt + n_tail * stride_wn,
                       mask=q_mask, other=0.0).to(tl.float32)
    o_tail_ptrs = (OUT + b * stride_ob + q_offs[:, None].to(tl.int64) * stride_ot
                   + n_tail * stride_on + d_offs[None, :].to(tl.int64) * stride_od)
    tl.store(
        o_tail_ptrs,
        (pdd_tail[:, None] + 1.0) * pv_tail + pw2_tail[:, None] * av_tail,
        mask=q_mask[:, None],
    )
    ...
    if D == 128 and HPG == 16:
        ...
        ni = (head_start + HPG - 2).to(tl.int64)
        v_ptrs = (V + b * stride_vb + k_offs[:, None].to(tl.int64) * stride_vt
                  + ni * stride_vn + d_offs[None, :].to(tl.int64) * stride_vd)
        v_blk = tl.load(v_ptrs, mask=k_mask[:, None], other=0.0)
        av = tl.dot(a_acc, v_blk)
...
class TritonDCOneKernel:
    """Post-only DC v2: Post1 plus one-store tail head."""
